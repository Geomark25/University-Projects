...
#define PRIORITY_QUEUES 5		//Oures toy sheduler
#define MAX_TIME 1000				//H megisth timh poy otan ftasei se auth tha kanei boost
...
//H spawn_thread dhmiourgei ena antistoixo tcb gia to pcb
//Prosthetoume 3o orisma enan deikth ptcb o opoios kanei pleon th syndesh TCB-PTCB
TCB* spawn_thread(PCB* pcb, PTCB *ptcb_ptr, void (*func)())
{
	...
	tcb->priority=0;	//Highest priotirty->0 , Lowest priority->4
...
{
	uint i=0;
	//We want to select the first thread existing at currently lowest not empty queue
  while(i< PRIORITY_QUEUES -1  && is_rlist_empty(&SCHED[i]) ){
    i++;
  }
...
//Periodically boosts all the threads in the system. 
//Katevazei ola ta threads kata ena priority outws wste na lythei to starvation problem.
void boost(){
	TCB* tcb =NULL;
	int i=0;
	//Arxizoume apo thn oura 0 kai kanoume boost 4 fores. 4->3, 3->2, 2->1, 1->0
	while(i<PRIORITY_QUEUES-1){
 		while (!is_rlist_empty(&(SCHED[i+1]))){              
	        tcb =rlist_pop_front(&(SCHED[i+1]))->tcb;
	        tcb->priority--;
	        rlist_push_back(&(SCHED[i]), &tcb->sched_node);
...
void yield(enum SCHED_CAUSE cause)
{
    ...
    counter_yield ++;
    //Kanoume boost otan o metrhths ftasei th timh pou exoume orisei gia MAX_TIME.
	  if(counter_yield==MAX_TIME) {
	    boost();
	    counter_yield = 0;
	  }
	...
	switch(cause){
		case SCHED_QUANTUM: 	//To nhma exei eksantlsisei to quantum alla den exei teleiwsei th douleia tou
			if (current->priority < PRIORITY_QUEUES - 1)
			{
				current->priority ++;
			}
			break;
		case SCHED_IO: //To nhma teleiwnei th douleia tou prin liksei to quantum
			if(current->priority > 0)
			{
				current->priority --;
			}
			break;
		case SCHED_MUTEX:		//Increase the priority of the thread by 1
		  if (current->priority < PRIORITY_QUEUES - 1 && current->curr_cause == current->last_cause)
			{
				current->priority ++;
			}
...
{
	//Kanoume to initialize gia tis oures 
