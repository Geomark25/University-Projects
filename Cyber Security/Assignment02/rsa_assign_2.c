...
void key_generation(int key_length) {
    ...
    if(mpz_invert(d,e,lambda) == 0){
        printf("Error: e and lambda not coprime!\n");
    ...
    mpz_clears(p,q,n,d,e,lambda,NULL);
    gmp_randclear(state);
    printf("Keys saved to public_key and private_key\n");
}
...
void encryption(const char *input_file, const char *output_file, const char *key_file){
    ...
    long size = ftell(fin);
    rewind(fin);
    if(size <= 0){ printf("Input file empty!\n"); exit(-1); }
...
void decryption(const char *input_file, const char *output_file, const char *key_file){
    ...
    size_t count;
    unsigned char *out = (unsigned char*)mpz_export(NULL,&count,1,1,0,0,m);
...
int main(int argc, char *argv[]){
    ...
    while((opt = getopt(argc, argv, "i:o:k:g:desva:h")) != -1){
        switch(opt){
            case 'i':
                if(!optarg){ 
                    printf("Error: -i requires an input file\n");
                    return -1; 
                }
                input_file = optarg;
                break;
            case 'o':
                if(!optarg){
                    printf("Error: -o requires an output file\n"); 
                    return -1;
                 }
                output_file = optarg;
                break;
            case 'k':
                if(!optarg){ 
                    printf("Error: -k requires a key file\n"); 
                    return -1;
                 }
                key_file = optarg;
                break;
            case 'g':
                if(!optarg){ 
                    printf("Error: -g requires key length\n"); 
                    return -1; 
                }
            ...
            case 'd':
                if(!input_file){
                     printf("Error: Decryption requires -i <input file>\n"); 
                     return -1;
                     }
                if(!output_file){
                     printf("Error: Decryption requires -o <output file>\n"); 
                     return -1;
                     }
                if(!key_file){ 
                    printf("Error: Decryption requires -k <private key>\n");
                     return -1;
                }
            ...
            case 'e':
                if(!input_file){ 
                    printf("Error: Encryption requires -i <input file>\n"); 
                    return -1;
                 }
                if(!output_file){
                     printf("Error: Encryption requires -o <output file>\n"); 
                     return -1;
                     }
                if(!key_file){ 
                    printf("Error: Encryption requires -k <public key>\n"); 
                    return -1;
                 }
            ...
            case 's':
                if(!input_file){
                     printf("Error: Signing requires -i <input file>\n"); 
                    return -1; 
                }
                if(!output_file){ 
                    printf("Error: Signing requires -o <signature file>\n");
                     return -1; 
                }
                if(!key_file){ 
                    printf("Error: Signing requires -k <private key>\n"); 
                return -1;
                 }
            ...
            case 'v':
                if(!input_file){ 
                    printf("Error: Verification requires -i <input file>\n");
                     return -1;
                 }
                if(!output_file){ 
                    printf("Error: Verification requires -o <signature file>\n"); 
                    return -1; 
                }
                if(!key_file){ 
                    printf("Error: Verification requires -k <public key>\n");
                     return -1; 
                }
            ...
            case 'a':
                if(!optarg){ 
                    printf("Error: -a requires a performance output file\n"); 
                    return -1;
                 }
                comparison(optarg);
                break;
