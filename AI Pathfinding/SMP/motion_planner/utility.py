...
def ensure_predictions(scenario: Scenario, ego_initial_state: State = None, ego_shape: Rectangle = None):
    """
    Ensure all dynamic obstacles have a prediction (dummy if missing).
    Returns a TrajectoryPrediction for the ego vehicle if provided.
    """
    # Patch all existing scenario dynamic obstacles
    for obj in scenario.dynamic_obstacles:
        if obj.prediction is None:
            dummy_traj = Trajectory(initial_time_step=obj.initial_state.time_step, state_list=[obj.initial_state])
            obj.prediction = TrajectoryPrediction(dummy_traj, obj.obstacle_shape)

    # Create ego prediction if needed
    if ego_initial_state and ego_shape:
        ego_traj = Trajectory(initial_time_step=ego_initial_state.time_step, state_list=[ego_initial_state])
...
def plot_search_scenario(scenario, initial_state, ego_shape, planning_problem, config: Type[PlotConfig]):
    plt.figure(figsize=(16.25, 5))
    plt.axis('equal')
    renderer = MPRenderer(plot_limits=[35, 100, -10.0, 10.0])

    # Patch all dynamic obstacles and create ego prediction
    ego_prediction = ensure_predictions(scenario, ego_initial_state=initial_state, ego_shape=ego_shape)

    draw_params = {'scenario': {'lanelet': {'facecolor': '#F8F8F8'}}}
    scenario.draw(renderer, draw_params=draw_params)

    # Create ego vehicle with prediction
...
def initial_visualization(scenario, initial_state, ego_shape, planning_problem, config: Type[PlotConfig], path_fig):
    if not config.JUPYTER_NOTEBOOK and config.DO_PLOT:
        ...
        if path_fig:
            ...
            plt.axis('off')
            try:
                plt.savefig(path_fig + 'initial_scenario.' + config.OUTPUT_FORMAT,
...
def plot_motion_primitive(mp: MotionPrimitive, color='red'):
    ...
    x = [state.position[0] for state in mp.trajectory.state_list]
    y = [state.position[1] for state in mp.trajectory.state_list]
...
def plot_primitive_path(mp: List[State], status: MotionPrimitiveStatus, plotting_params):
    plt.plot(mp[-1].position[0], mp[-1].position[1], color=plotting_params[status.value][0], marker='o', markersize=8,
             zorder=27)
    x = [state.position[0] for state in mp]
    y = [state.position[1] for state in mp]
...
def update_visualization(primitive: List[State], status: MotionPrimitiveStatus, dict_node_status: Dict[int, Tuple],
                         path_fig, config, count, time_pause=0.01):
...
def create_trajectory_from_list_states(list_paths_primitives: List[List[State]]) -> Trajectory:
    ...
    # remove duplicates (every 6 states)
