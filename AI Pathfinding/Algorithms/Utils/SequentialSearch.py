...
class SequentialSearch(SearchBaseClass, ABC):
    ...
    def get_path(self, final_node):
        path_coords = self.get_node_path(final_node)
        return "->".join([f"({round(x,2)}, {round(y,2)})" for x,y in path_coords])
    
    def format_results(self, name, node_final):
        if node_final is None:
            return f"{name}:\nVisited Nodes number: {self.visited_count}\nPath: No Path Found\nEstimated Cost: inf"

        path_str = self.get_path(node_final)
        cost = getattr(node_final, 'cost', 0)

        return (f"{name}:\n"
                f"Visited Nodes number: {self.visited_count}\n"
                f"Path: {path_str}\n"
                f"Estimated Cost: {round(cost, 2)}"
                )